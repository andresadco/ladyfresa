...
export default function App(){
  ...
  const[resTab,setResTab]=useState("resumen"); // resumen | gastos | recolecciones
  ...
  useEffect(()=>{
    if(view==="recoleccion"&&usuarioActual){
      setRForm(f=>f.quien===usuarioActual?f:{...f,quien:usuarioActual});
    }
  },[view,usuarioActual]);
  ...
  const handleFoto=async(e)=>{
    ...
    // Para PDFs guardamos la URL igual, la vista distingue por extensión
    setForm(f=>({...f,foto:data.publicUrl,fotoEsPdf:file.type==="application/pdf"}));
  ...
  if(!usuarioActual)return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:`linear-gradient(160deg,${ROSA},${ROSA_DARK})`,padding:24}}>
      <div style={{fontSize:56,marginBottom:8}}>🍓</div>
      <div style={{fontSize:26,fontWeight:900,color:BLANCO,marginBottom:4}}>Lady Fresa</div>
      <div style={{fontSize:14,color:"rgba(255,255,255,0.7)",marginBottom:36}}>¿Quién eres?</div>
      <div style={{width:"100%",maxWidth:320,display:"flex",flexDirection:"column",gap:12}}>
        {EQUIPO.map(nombre=>(
          <button key={nombre} onClick={()=>{
              if(PINES[nombre]){setPinUsuario(nombre);setPinInput("");setPinError(false);}
              else{localStorage.setItem("lf_usuario",nombre);setUsuarioActual(nombre);}
            }}
            style={{width:"100%",padding:"16px 0",borderRadius:16,
              border:nombre==="Apolo"?"2px solid #FFD700":PINES[nombre]?"2px solid rgba(255,255,255,0.6)":"2px solid rgba(255,255,255,0.3)",
              background:nombre==="Apolo"?"rgba(255,215,0,0.2)":PINES[nombre]?"rgba(255,255,255,0.25)":"rgba(255,255,255,0.15)",
              color:BLANCO,fontSize:17,fontWeight:800,
              cursor:"pointer",fontFamily:"inherit",backdropFilter:"blur(8px)",
              letterSpacing:0.3}}>
            {nombre==="Apolo"?"🧑 ":PINES[nombre]?"🔒 ":""}{nombre}{nombre==="Apolo"?" (Recolector)":""}
          </button>
        ))}
      </div>
      ...
      {pinUsuario&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:24}}>
          <div style={{background:BLANCO,borderRadius:24,padding:"32px 24px",width:"100%",maxWidth:320,textAlign:"center",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
            <div style={{fontSize:40,marginBottom:8}}>🔒</div>
            <div style={{fontSize:18,fontWeight:900,color:GRIS_DARK,marginBottom:4}}>Código de acceso</div>
            <div style={{fontSize:13,color:GRIS_TEXT,marginBottom:24}}>Ingresa el PIN de <strong>{pinUsuario}</strong></div>
            {/* Mostrar puntos */}
            <div style={{display:"flex",justifyContent:"center",gap:14,marginBottom:24}}>
              {[0,1,2,3].map(i=>(
                <div key={i} style={{width:16,height:16,borderRadius:"50%",
                  background:pinInput.length>i?ROSA:"#E0E0E0",
                  transition:"background 0.15s"}}/>
              ))}
            </div>
            {pinError&&<div style={{fontSize:12,color:"#E53935",fontWeight:700,marginBottom:12}}>❌ PIN incorrecto, intenta de nuevo</div>}
            {/* Teclado numérico */}
              ...
              {["1","2","3","4","5","6","7","8","9","","0","⌫"].map((k,i)=>(
                <button key={i} disabled={k===""} onClick={()=>{
                  if(k==="⌫"){setPinInput(p=>p.slice(0,-1));setPinError(false);}
                    ...
                    setPinInput(nuevo);
                    if(nuevo.length===4){
                      if(nuevo===PINES[pinUsuario]){
                        localStorage.setItem("lf_usuario",pinUsuario);
                        setUsuarioActual(pinUsuario);
                        setPinUsuario(null);setPinInput("");
                      } else {
                        setPinError(true);
                  ...
                  style={{padding:"18px 0",borderRadius:14,border:"1.5px solid #E8E8E8",
                    background:k===""?"transparent":GRIS_LIGHT,fontSize:20,fontWeight:700,
                    color:GRIS_DARK,cursor:k===""?"default":"pointer",fontFamily:"inherit",
                    opacity:k===""?0:1}}>
                  {k}
                </button>
              ))}
            </div>
            <button onClick={()=>{setPinUsuario(null);setPinInput("");setPinError(false);}}
              style={{width:"100%",padding:"12px 0",borderRadius:12,border:"1.5px solid #E0E0E0",
                background:GRIS_LIGHT,color:GRIS_MED,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
              Cancelar
            </button>
          </div>
        </div>
  ...
  if(view==="nuevo")return(
    <Screen title={editandoId?"Editar Gasto":"Nuevo Gasto"}
      ...
      <FL>📷 Foto o archivo del ticket</FL>
      {form.foto
        ?<div style={{position:"relative",marginBottom:8}}>
           {form.foto.match(/\.pdf(\?|$)/i)
             ?<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
                 padding:"14px",borderRadius:12,background:"#FFF8E1",border:"1px solid #FFE082"}}>
                <a href={form.foto} target="_blank" rel="noreferrer"
                   style={{fontSize:13,fontWeight:700,color:AMBAR,textDecoration:"none"}}>
                  📄 PDF adjunto — toca para ver
                </a>
                <button onClick={()=>setForm(f=>({...f,foto:null,fotoEsPdf:false}))}
                  style={{background:"none",border:"none",cursor:"pointer",fontSize:16,color:GRIS_TEXT}}>✕</button>
              </div>
             :<><img src={form.foto} alt="ticket" style={{width:"100%",borderRadius:12,objectFit:"cover",maxHeight:160}}/>
               <button onClick={()=>setForm(f=>({...f,foto:null,fotoEsPdf:false}))}
                 style={{position:"absolute",top:8,right:8,background:"rgba(0,0,0,0.5)",border:"none",
                   borderRadius:20,color:BLANCO,padding:"4px 10px",cursor:"pointer",fontSize:12}}>✕</button>
             </>
           }
         </div>
        :<div style={{display:"flex",gap:8,marginBottom:8}}>
           <input ref={camaraRef} type="file" accept="image/*" capture="environment" style={{display:"none"}} onChange={handleFoto}/>
           <input ref={fileRef} type="file" accept="image/*,application/pdf" style={{display:"none"}} onChange={handleFoto}/>
           <button onClick={()=>camaraRef.current.click()} disabled={uploadingFoto}
             style={{flex:1,padding:"13px 0",borderRadius:13,border:`1.5px solid ${ROSA}`,background:ROSA_BG,
               color:ROSA,fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit",
               opacity:uploadingFoto?0.6:1}}>
             📷 Cámara
           </button>
           <button onClick={()=>fileRef.current.click()} disabled={uploadingFoto}
             style={{flex:1,padding:"13px 0",borderRadius:13,border:"1.5px solid #E0E0E0",background:GRIS_LIGHT,
               color:GRIS_MED,fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit",
               opacity:uploadingFoto?0.6:1}}>
             {uploadingFoto?"⏳ Subiendo…":"🖼 Galería / PDF"}
           </button>
         </div>
      }
  ...
  if(view==="resumen"){if(!PUEDE_VER_NUMEROS.includes(usuarioActual))return null;
    ...
    // Desglose por método de pago (efectivo vs otros)
    const tgEfectivo=listaCompleta.filter(g=>g.forma==="Efectivo").reduce((s,g)=>s+num(g.monto),0);
    const tgOtros=tg-tgEfectivo;
    ...
    // ── COMPARACIÓN MES ANTERIOR (para mostrar tendencia ▲▼) ──
    const mkPrev=(()=>{const [y,m]=mk.split("-").map(Number);const d=new Date(y,m-2,1);return monthKey(d.toISOString());})();
    const tgPrev=totG(mkPrev);
    const tvPrev=ventas.filter(v=>monthKey(v.fecha)===mkPrev).reduce((s,v)=>s+num(v.efectivo),0);
    const diff=(curr,prev)=>{if(!prev)return null;const pct=Math.round(((curr-prev)/prev)*100);return{pct,up:pct>=0};};
    const diffVentas=diff(totalVentasMes,tvPrev);
    ...
    return(
      <Screen title={`Resumen ${monthLabel(mk)}`} onBack={()=>setView("inicio")}
        ...
        <div style={{marginBottom:14}}>
          ...
          {/* ═══ VALIDACIÓN: ¿Dónde está el efectivo neto? ═══ */}
          {(()=>{
            const efectivoNeto=totalVentasMes-tgEfectivo;
            const suma=totalRecolectadoMes+pendienteMes+faltantesMes;
            const descuadre=Math.round(efectivoNeto-suma);
            const cuadra=Math.abs(descuadre)<1;
            return(
              <div style={{background:BLANCO,borderRadius:14,padding:"12px 14px",marginBottom:10,boxShadow:"0 2px 8px rgba(0,0,0,0.05)",border:`1px solid ${cuadra?VERDE+"33":"#E5393333"}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <div style={{fontSize:10,color:GRIS_TEXT,fontWeight:800,textTransform:"uppercase",letterSpacing:0.5}}>¿Dónde está el efectivo neto?</div>
                  <div style={{background:cuadra?VERDE_BG:"#FFEBEE",color:cuadra?VERDE:"#E53935",fontSize:10,padding:"2px 8px",borderRadius:6,fontWeight:800}}>
                    {cuadra?"✅ Cuadra":`⚠️ ${descuadre>0?"Falta":"Sobra"} ${fmtMXN(Math.abs(descuadre))}`}
                  </div>
                </div>
                <div style={{display:"flex",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #f0f0f0"}}>
                  <div style={{width:4,height:22,background:AZUL,borderRadius:2,marginRight:10}}/>
                  <div style={{flex:1}}>
                    <div style={{fontSize:12,color:GRIS_DARK,fontWeight:700}}>Ya recolectado</div>
                    <div style={{fontSize:10,color:GRIS_TEXT}}>En tus manos</div>
                  </div>
                  <div style={{fontSize:15,fontWeight:900,color:AZUL}}>{fmtMXN(totalRecolectadoMes)}</div>
                </div>
                <div style={{display:"flex",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #f0f0f0"}}>
                  <div style={{width:4,height:22,background:AMBAR,borderRadius:2,marginRight:10}}/>
                  <div style={{flex:1}}>
                    <div style={{fontSize:12,color:GRIS_DARK,fontWeight:700}}>Pendiente recolectar</div>
                    <div style={{fontSize:10,color:GRIS_TEXT}}>{diasMesPendientes.length} día{diasMesPendientes.length!==1?"s":""} en el local</div>
                  </div>
                  <div style={{fontSize:15,fontWeight:900,color:AMBAR}}>{fmtMXN(pendienteMes)}</div>
                </div>
                <div style={{display:"flex",alignItems:"center",padding:"6px 0"}}>
                  <div style={{width:4,height:22,background:"#E53935",borderRadius:2,marginRight:10}}/>
                  <div style={{flex:1}}>
                    <div style={{fontSize:12,color:GRIS_DARK,fontWeight:700}}>Faltantes</div>
                    <div style={{fontSize:10,color:GRIS_TEXT}}>Diferencias del mes</div>
                  </div>
                  <div style={{fontSize:15,fontWeight:900,color:"#E53935"}}>{fmtMXN(faltantesMes)}</div>
                </div>
                <div style={{borderTop:"1px dashed #ddd",marginTop:6,paddingTop:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:11,color:GRIS_TEXT}}>Efectivo neto del mes</span>
                  <span style={{fontSize:13,fontWeight:900,color:GRIS_DARK}}>{fmtMXN(efectivoNeto)}</span>
                </div>
              </div>
            );
          })()}
        ...
        {/* ═══ BARRA DE TABS ═══ */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:14,background:GRIS_LIGHT,padding:4,borderRadius:12}}>
          {[
            {id:"resumen",icon:"📊",label:"Resumen"},
            {id:"gastos",icon:"💸",label:`Gastos ${lista.length>0?`(${lista.length})`:""}`},
            {id:"recolecciones",icon:"💰",label:"Cobranza"},
          ].map(t=>(
            <button key={t.id} onClick={()=>setResTab(t.id)}
              style={{
                padding:"10px 4px",borderRadius:9,border:"none",cursor:"pointer",fontFamily:"inherit",
                background:resTab===t.id?BLANCO:"transparent",
                boxShadow:resTab===t.id?"0 2px 6px rgba(0,0,0,0.08)":"none",
                color:resTab===t.id?ROSA:GRIS_MED,
        ...
        {/* ═══════════════════════════════════════════════════ */}
        {/* TAB 1: RESUMEN — vista de un vistazo */}
        {/* ═══════════════════════════════════════════════════ */}
        {resTab==="resumen"&&(
          <>
            {/* Alertas rápidas */}
            ...
            {tvPrev>0&&(
              <>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
                  <div style={{background:BLANCO,borderRadius:12,padding:"12px 14px",boxShadow:"0 2px 8px rgba(0,0,0,0.05)",borderTop:`3px solid ${VERDE}`}}>
                    <div style={{fontSize:10,color:VERDE,fontWeight:800,marginBottom:4}}>💵 VENTAS</div>
                    <div style={{fontSize:17,fontWeight:900,color:VERDE}}>{fmtMXN(totalVentasMes)}</div>
                    <div style={{fontSize:10,color:GRIS_TEXT,marginTop:2}}>{fmtMXN(tvPrev)} mes pasado</div>
                    {diffVentas&&<div style={{fontSize:11,fontWeight:800,color:diffVentas.up?VERDE:"#E53935",marginTop:2}}>{diffVentas.up?"▲":"▼"} {Math.abs(diffVentas.pct)}%</div>}
                  </div>
                  <div style={{background:BLANCO,borderRadius:12,padding:"12px 14px",boxShadow:"0 2px 8px rgba(0,0,0,0.05)",borderTop:`3px solid ${ROSA}`}}>
                    <div style={{fontSize:10,color:ROSA,fontWeight:800,marginBottom:4}}>🛒 GASTOS</div>
                    <div style={{fontSize:17,fontWeight:900,color:ROSA}}>{fmtMXN(tg)}</div>
                    <div style={{fontSize:10,color:GRIS_TEXT,marginTop:2}}>{fmtMXN(tgPrev)} mes pasado</div>
                    {diffGastos&&<div style={{fontSize:11,fontWeight:800,color:diffGastos.up?"#E53935":VERDE,marginTop:2}}>{diffGastos.up?"▲":"▼"} {Math.abs(diffGastos.pct)}%</div>}
                  </div>
                </div>
              </>
            )}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:8}}>
              <button onClick={()=>setResTab("gastos")}
                style={{background:BLANCO,border:"none",borderRadius:14,padding:"14px",boxShadow:"0 2px 8px rgba(0,0,0,0.05)",cursor:"pointer",fontFamily:"inherit",textAlign:"left",borderLeft:`3px solid ${ROSA}`}}>
                <div style={{fontSize:11,color:GRIS_TEXT,fontWeight:800,textTransform:"uppercase",letterSpacing:0.4}}>💸 Ver gastos</div>
                <div style={{fontSize:15,fontWeight:900,color:ROSA,marginTop:4}}>{listaCompleta.length} este mes</div>
                <div style={{fontSize:10,color:GRIS_TEXT,marginTop:2}}>Toca para detalle →</div>
              </button>
              <button onClick={()=>setResTab("recolecciones")}
                style={{background:BLANCO,border:"none",borderRadius:14,padding:"14px",boxShadow:"0 2px 8px rgba(0,0,0,0.05)",cursor:"pointer",fontFamily:"inherit",textAlign:"left",borderLeft:`3px solid ${AZUL}`}}>
                <div style={{fontSize:11,color:GRIS_TEXT,fontWeight:800,textTransform:"uppercase",letterSpacing:0.4}}>💰 Cobranza</div>
                <div style={{fontSize:15,fontWeight:900,color:AZUL,marginTop:4}}>{recoleccionesMes.length} recolecciones</div>
                <div style={{fontSize:10,color:GRIS_TEXT,marginTop:2}}>Toca para detalle →</div>
              </button>
            </div>
          </>
        ...
        {/* ═══════════════════════════════════════════════════ */}
        {/* TAB 2: GASTOS — todo lo de gastos en un solo lugar */}
        {/* ═══════════════════════════════════════════════════ */}
        {resTab==="gastos"&&(
          <>
            ...
            {tg>0&&(
              <>
                ...
                <div style={{background:BLANCO,borderRadius:14,padding:"14px",marginBottom:14,boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
                  {Object.entries(porForma).sort((a,b)=>b[1]-a[1]).map(([forma,monto])=>{
                    const pct=tg?Math.round((monto/tg)*100):0;
                    const esEfectivo=forma==="Efectivo";
                    const color=esEfectivo?VERDE:AZUL;
                    const emoji=esEfectivo?"💵":forma.toLowerCase().includes("tarjeta")?"💳":forma.toLowerCase().includes("transfer")?"🏦":forma.toLowerCase().includes("mercado")?"📱":"💰";
                    return(
                      <div key={forma} style={{marginBottom:10}}>
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:4}}>
                          <span style={{color:GRIS_DARK,fontWeight:700}}>{emoji} {forma}</span>
                          <span style={{color,fontWeight:800}}>{fmtMXN(monto)} <span style={{color:GRIS_TEXT,fontWeight:600,fontSize:11}}>({pct}%)</span></span>
                        </div>
                        <div style={{height:5,background:"#F0F0F0",borderRadius:3,overflow:"hidden"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:GRIS_TEXT,marginTop:8,paddingTop:10,borderTop:"1px solid #F0F0F0"}}>
                    <span>💵 Efectivo: <strong style={{color:VERDE}}>{fmtMXN(tgEfectivo)}</strong></span>
                    <span>💳 Otros: <strong style={{color:AZUL}}>{fmtMXN(tgOtros)}</strong></span>
                  </div>
                </div>
              </>
            )}
            ...
            {/* Filtro persona */}
            <div style={{fontSize:11,color:GRIS_TEXT,fontWeight:800,marginBottom:6,textTransform:"uppercase",letterSpacing:0.4}}>Persona</div>
            ...
            <button onClick={()=>setResFiltrosOpen(!resFiltrosOpen)}
              style={{width:"100%",marginBottom:resFiltrosOpen?0:14,background:GRIS_LIGHT,border:"none",borderRadius:resFiltrosOpen?"10px 10px 0 0":10,padding:"8px 14px",fontSize:11,fontWeight:800,color:GRIS_MED,cursor:"pointer",fontFamily:"inherit",textTransform:"uppercase",letterSpacing:0.4,textAlign:"left"}}>
              📅 Rango de fechas {resFiltrosOpen?"▲":"▼"}
              {(resDesde||resHasta)&&<span style={{marginLeft:8,fontSize:10,color:ROSA}}>● activo</span>}
            </button>
            {resFiltrosOpen&&(
              <div style={{marginBottom:14,background:GRIS_LIGHT,borderRadius:"0 0 10px 10px",padding:"4px 14px 12px"}}>
            ...
            {(resCat!=="todos"||filtroQ!=="todos"||resBuscar||resDesde||resHasta)&&(
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:ROSA_BG,padding:"10px 14px",borderRadius:12,marginBottom:14,border:`1px solid ${ROSA}33`}}>
                <div style={{fontSize:12,color:ROSA,fontWeight:800}}>
                  📊 {lista.length}/{gMes(mk).length} · <strong>{fmtMXN(tg)}</strong>
                </div>
        ...
        {/* ═══════════════════════════════════════════════════ */}
        {/* TAB 3: RECOLECCIONES — toda la cobranza */}
        {/* ═══════════════════════════════════════════════════ */}
        {resTab==="recolecciones"&&(
          <>
            {/* Resumen recolectado */}
            <div style={{background:BLANCO,borderRadius:14,padding:"14px",marginBottom:14,boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
              <div style={{display:"flex",justifyContent:"space-between",paddingBottom:10,marginBottom:10,borderBottom:"1px solid #F0F0F0"}}>
                <div>
                  ...
                  <div style={{fontSize:11,color:GRIS_TEXT,marginTop:2}}>{recoleccionesMes.length} recolección{recoleccionesMes.length!==1?"es":""}</div>
                </div>
                {faltantesMes>0&&(
                  <div style={{textAlign:"right"}}>
                ...
                return(
                  <div key={nombre} style={{marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:3}}>
                      ...
                      <span style={{color:VERDE,fontWeight:800}}>{fmtMXN(monto)} <span style={{color:GRIS_TEXT,fontWeight:600,fontSize:11}}>({pct}%)</span></span>
                    </div>
                    <div style={{height:5,background:"#F0F0F0",borderRadius:3,overflow:"hidden"}}>
                      <div style={{height:"100%",width:`${pct}%`,background:VERDE,borderRadius:3}}/>
                    </div>
                  </div>
                );
              })}
            ...
            {diasMesPendientes.length>0&&(
              <div style={{background:VERDE_BG,border:`1px solid ${VERDE}`,borderRadius:12,padding:"12px 14px",marginBottom:14}}>
                <div style={{fontSize:11,color:VERDE,fontWeight:800,textTransform:"uppercase",letterSpacing:0.4}}>⏳ Pendiente de recolectar</div>
                <div style={{fontSize:20,fontWeight:900,color:VERDE,marginTop:4}}>{fmtMXN(pendienteMes)}</div>
                <div style={{fontSize:11,color:VERDE,marginTop:2}}>{diasMesPendientes.length} día{diasMesPendientes.length!==1?"s":""} sin recolectar</div>
              </div>
            )}
            ...
            {ventasMes.filter(v=>v.nota).length>0&&(
              <>
                ...
                {ventasMes.filter(v=>v.nota).map(v=>(
            ...
            {/* Lista de recolecciones del mes */}
                ...
                {recoleccionesMes.sort((a,b)=>b.fecha_recoleccion.localeCompare(a.fecha_recoleccion)).map(r=>(
                  <button key={r.id} onClick={()=>{setSelRec(r);setView("detalle-recoleccion");}}
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      ...
                      <div style={{textAlign:"right"}}>
                        <div style={{fontSize:15,fontWeight:900,color:VERDE}}>{fmtMXN(r.monto_fisico??r.monto_total)}</div>
                        {num(r.faltante)>0&&<div style={{fontSize:11,color:"#E53935",fontWeight:700}}>−{fmtMXN(r.faltante)}</div>}
                      </div>
                    </div>
                  </button>
                ))}
            ...
            {recoleccionesMes.length===0&&<Empty>Sin recolecciones este mes</Empty>}
          </>
        )}
      </Screen>
    );
  ...
  if(view==="analitica"){if(!PUEDE_VER_NUMEROS.includes(usuarioActual))return null;
    ...
    // Desglose gastos del mes por método (efectivo vs otros)
    const tgEfectivoM=gMes(mk).filter(g=>g.forma==="Efectivo").reduce((s,g)=>s+num(g.monto),0);
    const tgOtrosM=tgM-tgEfectivoM;
    const balanceEfectivo=tvM-tgEfectivoM;
      ...
      <Screen title="Tendencias 📉" onBack={()=>setView("inicio")} action={<ExportBtn onClick={()=>exportExcel(gastos,ventas,recolecciones,null)} label="📥 Excel"/>}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          {[
            ...
            {label:"💵 Gastos efectivo",val:tgEfectivoM,color:ROSA},
            {label:"💳 Gastos otros",val:tgOtrosM,color:AZUL},
            {label:`${balance>=0?"✅":"🔴"} Balance total`,val:balance,color:balance>=0?VERDE:"#E53935"},
            {label:`${balanceEfectivo>=0?"✅":"🔴"} Balance efectivo`,val:balanceEfectivo,color:balanceEfectivo>=0?VERDE:"#E53935"},
          ].map(k=>(
        ...
        <div style={{background:BLANCO,borderRadius:16,padding:"18px",marginBottom:16,boxShadow:"0 2px 10px rgba(0,0,0,0.06)",border:`1.5px solid ${ROSA}22`}}>
          ...
          {[
            ...
            {label:"Gastos en efectivo",val:fmtMXN(tgEfectivoM),color:ROSA},
            {label:"Gastos con tarjeta / otros",val:fmtMXN(tgOtrosM),color:AZUL},
            {label:"Total gastos operativos",val:fmtMXN(tgM),color:ROSA},
            {label:"Balance neto (total)",val:fmtMXN(balance),color:balance>=0?VERDE:"#E53935"},
            {label:"Balance efectivo (caja)",val:fmtMXN(balanceEfectivo),color:balanceEfectivo>=0?VERDE:"#E53935"},
  ...
  return(
    <div style={S.root}>
      <div style={S.header}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {PUEDE_VER_NUMEROS.includes(usuarioActual)&&(creditosPendientes.length>0)&&(
          ...
          <button onClick={()=>{localStorage.removeItem("lf_usuario");setUsuarioActual(null);}}
            style={{background:ROSA_BG,border:`1.5px solid ${ROSA}`,borderRadius:10,padding:"5px 10px",
              fontSize:11,fontWeight:800,color:ROSA,cursor:"pointer"}}>
            👤 {usuarioActual}
          </button>
        </div>
      </div>
      ...
      <div style={S.hero}>
        ...
        {PUEDE_VER_NUMEROS.includes(usuarioActual)
          ...
          :<>
            <div style={{fontSize:28,fontWeight:900,color:"rgba(255,255,255,0.9)",marginTop:8}}>
              {todayG.length > 0 ? `${todayG.length} gasto${todayG.length!==1?"s":""} registrado${todayG.length!==1?"s":""}` : "Sin gastos hoy"}
            </div>
            <div style={{fontSize:13,color:"rgba(255,255,255,0.6)",marginTop:6}}>Hola, {usuarioActual} 👋</div>
          </>
        }
      </div>
          ...
          {g.foto&&showFoto&&(
            g.foto.match(/\.pdf(\?|$)/i)
              ?<a href={g.foto} target="_blank" rel="noreferrer"
                  style={{display:"flex",alignItems:"center",gap:6,marginTop:6,padding:"10px 12px",
                    background:"#FFF8E1",borderRadius:8,border:"1px solid #FFE082",
                    fontSize:12,fontWeight:700,color:AMBAR,textDecoration:"none"}}>
                  📄 Abrir PDF del ticket
                </a>
              :<img src={g.foto} alt="ticket" style={{width:"100%",borderRadius:8,marginTop:6,objectFit:"cover",maxHeight:140}}/>
          )}
        </div>
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,marginLeft:4}}>
          {canSeeNumbers&&<div style={{fontWeight:900,color:cat.color,fontSize:15}}>{fmtMXN(g.monto)}</div>}
          {canEdit?(
            ...
            <div style={{fontSize:9,color:"#CCC",fontWeight:600,marginTop:2}}>🔒</div>
          )}
        </div>
      </div>
    </div>
